...
def build_actual(path: Path) -> None:
    ...
    rows = [
        ...
        ("東京第一倉庫", "CCC-300", 4),      # SAP に無し → 預り書のみ
...
def build_sap(path: Path) -> None:
    ...
    rows = [
        ...
        ("住友商事マシネックス株式会社", "ZZZ-900", 30),
        ("住友商事マシネックス株式会社", "ZZZ-901", 12),
        ("XX01", "YYY-800", 4),
    ]
...
def build_mapping(path: Path) -> None:
    ...
    for before, after in [
        ...
        ("ST03", "福岡倉庫"),
        ("ST04", "名古屋倉庫"),
        ("XX01", "住友商事マシネックス株式会社"),  # 変換後も預り書に無い倉庫
